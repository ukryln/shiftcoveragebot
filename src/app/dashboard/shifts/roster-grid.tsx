...
type RawShift = { id: string; staff_id: string; start_time: string; end_time: string };

type CellState = { id: string | null; text: string; invalid: boolean };

function emptyRow(length: number): CellState[] {
  return Array.from({ length }, () => ({ id: null, text: "", invalid: false }));
}
    ...
    setGrid((prev) => ({
      ...
      [staffMember.id]: {
        ...
        [period]: prev[staffMember.id][period].map((c, i) => {
          ...
          if (!result.id) return { id: null, text: "", invalid: false };
          return { id: result.id, text: formatCellText(startTimeIso!, endTimeIso!), invalid: false };
        }),
      },
    }));
  }
  ...
  return (
    <div className="mt-4 overflow-x-auto">
      <table className="border-collapse text-sm">
        <thead>
          <tr>
            <th className="w-28 border border-gray-200 bg-gray-50 p-2 text-left text-xs text-gray-500">
              Staff
            </th>
            <th className="w-16 border border-gray-200 bg-gray-50 p-2"></th>
            {weekDates.map((dateISO) => (
              <th
                key={dateISO}
                className="w-20 border border-gray-200 bg-gray-50 p-2 text-center text-xs text-gray-500"
              >
                {dayLabel(dateISO)}
              </th>
            ))}
            <th className="w-16 border border-gray-200 bg-gray-100 p-2 text-center text-xs text-gray-500">
              Total
            </th>
          </tr>
        </thead>
          ...
          {staffList.map((staffMember) => {
            ...
            return (
              <Fragment key={staffMember.id}>
                <tr>
                  <td rowSpan={3} className="border border-gray-200 p-2 align-top font-medium text-gray-900">
                    {staffMember.name}
                    <div className="text-xs font-normal text-gray-500">{staffMember.role}</div>
                  </td>
                  <td className="border border-gray-200 bg-gray-50 p-2 text-xs text-gray-500">LUNCH</td>
                    ...
                    <td key={i} className="border border-gray-200 p-1">
                      <input
                        ...
                        className={`w-16 rounded border px-1 py-0.5 text-center ${
                          cell.invalid ? "border-red-400 bg-red-50" : "border-transparent"
                        } focus:border-gray-300`}
                      />
                    </td>
                  ))}
                  <td rowSpan={3} className="border border-gray-200 bg-gray-50 p-2 text-center align-middle font-semibold text-gray-800">
                    {weekTotal || ""}
                  </td>
                </tr>
                <tr>
                  <td className="border border-gray-200 bg-gray-50 p-2 text-xs text-gray-500">DINNER</td>
                ...
                <tr>
                  <td className="border border-gray-200 bg-orange-50 p-2 text-xs font-semibold text-gray-700">
                    TOTAL
                  </td>
                  {dayTotals.map((total, i) => (
                    <td
                      key={i}
                      className="border border-gray-200 bg-orange-50 p-2 text-center font-semibold text-gray-800"
                    >
                      {total || 0}
                    </td>
                  ))}
